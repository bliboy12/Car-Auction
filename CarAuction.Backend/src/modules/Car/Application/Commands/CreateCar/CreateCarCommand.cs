...
public class CreateCarCommand : IRequest<Guid>
{
    ...
    public CreateCarCommand(string brand, string model, string trim, int year, int kilometers, bool hasDamage, string description, string color, string fuel, Guid sellerId)
    {
        Brand = brand;
        Model = model;
        Trim = trim;
